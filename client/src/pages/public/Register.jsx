  ...
  const Field = ({ name, label, type = 'text', icon: Icon, placeholder, autoComplete }) => (
    <div>
      ...
      <div className="relative">
        ...
        <input
          name={name} type={name === 'password' ? (showPw ? 'text' : 'password') : type}
          autoComplete={autoComplete} value={form[name]} onChange={handleChange}
          placeholder={placeholder}
          className={`form-input pl-10 ${errors[name] ? 'border-rose-400 focus:border-rose-500' : ''}`}
        />
        {name === 'password' && (
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex flex-col items-center gap-3">
            ...
            <p className="font-display text-2xl font-semibold" style={{ color: 'var(--text-primary)' }}>Sable Grand</p>
          </Link>
        ...
        <div className="card card-body shadow-card-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Field name="firstName" label="First Name" icon={User} placeholder="Sipho" autoComplete="given-name" />
              <Field name="lastName"  label="Last Name"  icon={User} placeholder="Dlamini" autoComplete="family-name" />
            </div>
            <Field name="email" label="Email Address" type="email" icon={Mail} placeholder="you@example.co.za" autoComplete="email" />
            <Field name="phone" label="Mobile Number" type="tel" icon={Phone} placeholder="+27 82 000 0000" autoComplete="tel" />
            <Field name="password" label="Password" icon={Lock} placeholder="Min. 8 characters" autoComplete="new-password" />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input name="confirm" type="password" value={form.confirm} onChange={handleChange}
                  ...
                  className={`form-input pl-10 ${errors.confirm ? 'border-rose-400' : ''}`} />
              </div>
